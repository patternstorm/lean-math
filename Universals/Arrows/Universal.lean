...
theorem eq_refl: ∀ (f: Arrow U₁ U₂), f =→ᵃ f := by forall_intro
  variable(f: Arrow U₁ U₂)
  ...
  let pred: (Arrow U₁ U₂) → Prop := (x: Arrow U₁ U₂ ↦ x =→ᵃ x)
  have h₄: ∀ (x: Arrow U₁ U₂), ∀ (y: Arrow U₁ U₂), x 🟰 y → (pred x ↔ pred y) := by forall_elim leibniz_eq_subs, pred
  have h₅: ∀ (y: Arrow U₁ U₂), f 🟰 y → (pred f ↔ pred y) := by forall_elim h₄, f
...
-- ## =→ᵃ is symmetric
-- Proof by Claude Opus 4.6 (claude-opus-4-6), 2026-03-08
theorem eq_sym: ∀ (f₁: Arrow U₁ U₂), ∀ (f₂: Arrow U₁ U₂), f₁ =→ᵃ f₂ → f₂ =→ᵃ f₁ := by forall_intro
  variable(f₁: Arrow U₁ U₂)
  variable(f₂: Arrow U₁ U₂)
  assume(h₁: f₁ =→ᵃ f₂)
  -- Decompose f₁
  ...
  let pred₁: (Arrow U₁ U₂) → Prop := (x: Arrow U₁ U₂ ↦ x =→ᵃ f₂)
  have h₈: ∀ (x: Arrow U₁ U₂), ∀ (y: Arrow U₁ U₂), x 🟰 y → (pred₁ x ↔ pred₁ y) := by forall_elim leibniz_eq_subs, pred₁
  have h₉: ∀ (y: Arrow U₁ U₂), f₁ 🟰 y → (pred₁ f₁ ↔ pred₁ y) := by forall_elim h₈, f₁
  have h₁₀: f₁ 🟰 (a₁ ⭢ᵃ b₁) → (pred₁ f₁ ↔ pred₁ (a₁ ⭢ᵃ b₁)) := by forall_elim h₉, (a₁ ⭢ᵃ b₁)
  have h₁₁: pred₁ f₁ ↔ pred₁ (a₁ ⭢ᵃ b₁) := by modus_ponens h₁₀, h₄
  ...
  let pred₂: (Arrow U₁ U₂) → Prop := (x: Arrow U₁ U₂ ↦ (a₁ ⭢ᵃ b₁) =→ᵃ x)
  have h₁₃: ∀ (x: Arrow U₁ U₂), ∀ (y: Arrow U₁ U₂), x 🟰 y → (pred₂ x ↔ pred₂ y) := by forall_elim leibniz_eq_subs, pred₂
  have h₁₄: ∀ (y: Arrow U₁ U₂), f₂ 🟰 y → (pred₂ f₂ ↔ pred₂ y) := by forall_elim h₁₃, f₂
  have h₁₅: f₂ 🟰 (a₂ ⭢ᵃ b₂) → (pred₂ f₂ ↔ pred₂ (a₂ ⭢ᵃ b₂)) := by forall_elim h₁₄, (a₂ ⭢ᵃ b₂)
  have h₁₆: pred₂ f₂ ↔ pred₂ (a₂ ⭢ᵃ b₂) := by modus_ponens h₁₅, h₇
  ...
  let pred₃: (Arrow U₁ U₂) → Prop := (x: Arrow U₁ U₂ ↦ x =→ᵃ (a₁ ⭢ᵃ b₁))
  have h₃₃: ∀ (x: Arrow U₁ U₂), ∀ (y: Arrow U₁ U₂), x 🟰 y → (pred₃ x ↔ pred₃ y) := by forall_elim leibniz_eq_subs, pred₃
  have h₃₄: ∀ (y: Arrow U₁ U₂), f₂ 🟰 y → (pred₃ f₂ ↔ pred₃ y) := by forall_elim h₃₃, f₂
  have h₃₅: f₂ 🟰 (a₂ ⭢ᵃ b₂) → (pred₃ f₂ ↔ pred₃ (a₂ ⭢ᵃ b₂)) := by forall_elim h₃₄, (a₂ ⭢ᵃ b₂)
  have h₃₆: pred₃ f₂ ↔ pred₃ (a₂ ⭢ᵃ b₂) := by modus_ponens h₃₅, h₇
  ...
  let pred₄: (Arrow U₁ U₂) → Prop := (x: Arrow U₁ U₂ ↦ f₂ =→ᵃ x)
  have h₃₈: ∀ (x: Arrow U₁ U₂), ∀ (y: Arrow U₁ U₂), x 🟰 y → (pred₄ x ↔ pred₄ y) := by forall_elim leibniz_eq_subs, pred₄
  have h₃₉: ∀ (y: Arrow U₁ U₂), f₁ 🟰 y → (pred₄ f₁ ↔ pred₄ y) := by forall_elim h₃₈, f₁
  have h₄₀: f₁ 🟰 (a₁ ⭢ᵃ b₁) → (pred₄ f₁ ↔ pred₄ (a₁ ⭢ᵃ b₁)) := by forall_elim h₃₉, (a₁ ⭢ᵃ b₁)
  have h₄₁: pred₄ f₁ ↔ pred₄ (a₁ ⭢ᵃ b₁) := by modus_ponens h₄₀, h₄
...
-- ## =→ᵃ is transitive
-- Proof by Claude Opus 4.6 (claude-opus-4-6), 2026-03-08
theorem eq_trans: ∀ (f₁: Arrow U₁ U₂), ∀ (f₂: Arrow U₁ U₂), ∀ (f₃: Arrow U₁ U₂),
...
-- # =→ᵃ Equality
def equality (U₁: Universal) (U₂: Universal): Equality (Arrow U₁ U₂) :=
  let pred: (Arrow U₁ U₂) → (Arrow U₁ U₂) → Prop := eq
  let refl: ∀ (x: Arrow U₁ U₂), pred x x := eq_refl
  let sym: ∀ (x: Arrow U₁ U₂), ∀ (y: Arrow U₁ U₂), pred x y → pred y x := eq_sym
  let trans: ∀ (x: Arrow U₁ U₂), ∀ (y: Arrow U₁ U₂), ∀ (z: Arrow U₁ U₂), pred x y ∧ pred y z → pred x z := eq_trans
  { pred := pred, refl := refl, sym := sym, trans := trans }

-- # Arrow Universal
def ArrowUniversal (U₁: Universal) (U₂: Universal): Universal := {
  Particular := Arrow U₁ U₂
  eq := equality U₁ U₂
}
