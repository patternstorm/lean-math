...
noncomputable def singleton_of_operation: CongruentUnaryOperation U (𝐒𝐢𝐧𝐠𝐥𝐞𝐭𝐨𝐧𝐒𝐞𝐭 U) :=
  ...
  let cong: ∀ (a: U.Particular), ∀ (b: U.Particular), a =₍U₎ b → ({a}ₛₑₜ =₍𝐒𝐞𝐭 U₎ {b}ₛₑₜ) := by forall_intro
    ...
    have h₃: ∀ (S₂: Set U), {a}ₛₑₜ =₍𝐒𝐞𝐭 U₎ S₂ ↔ (∀ (x: U.Particular), x ∈ₛₑₜ {a}ₛₑₜ ↔ x ∈ₛₑₜ S₂) := by forall_elim set_extensionality (U := U), {a}ₛₑₜ
    have h₄: {a}ₛₑₜ =₍𝐒𝐞𝐭 U₎ {b}ₛₑₜ ↔ (∀ (x: U.Particular), x ∈ₛₑₜ {a}ₛₑₜ ↔ x ∈ₛₑₜ {b}ₛₑₜ) := by forall_elim h₃, {b}ₛₑₜ
    ...
    have h₇: {a}ₛₑₜ =₍𝐒𝐞𝐭 U₎ {b}ₛₑₜ := PC₀.deductive_eq_r2l h₄ h₆
    iterate h₇
  { op := op, cong := cong }
