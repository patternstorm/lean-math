...
noncomputable def singleton_elem_operation: CongruentUnaryOperation (𝐒𝐢𝐧𝐠𝐥𝐞𝐭𝐨𝐧𝐒𝐞𝐭 U) U :=
  ...
  let cong: ∀ (S₁: SingletonSet U), ∀ (S₂: SingletonSet U), ↑S₁ =ₛₑₜ ↑S₂ → (⊙ S₁ =₍U₎ ⊙ S₂) := by forall_intro
    ...
    -- Set extensionality
    have h₃: ∀ (S: Set U), ↑S₁ =ₛₑₜ S ↔ (∀ (x: U.Particular), x ∈ₛₑₜ ↑S₁ ↔ x ∈ₛₑₜ S) := by forall_elim set_extensionality, ↑S₁
