...
--
-- Congruence is proven inline below. The body `∃!₍U₎ x, x ∈ₛₑₜ S` varies in
-- S through the membership predicate; we unpack via `Sets.set_extensionality`
-- (to lift `S =ₛₑₜ S'` to pointwise membership equivalence) and
-- `exists_unique_def` (to convert `∃!₍U₎` to a plain ∃ + uniqueness),
-- repackaging on the other side.
...
unary_predicate is_singleton : (S : (𝐒𝐞𝐭 U).Particular ↦ ∃!₍U₎ (x: U.Particular), x ∈ₛₑₜ S)
  with cong:
    ∀ (S: Set U), ∀ (S': Set U), S =ₛₑₜ S' → ((∃!₍U₎ (x: U.Particular), x ∈ₛₑₜ S) ↔ (∃!₍U₎ (x: U.Particular), x ∈ₛₑₜ S')) := by forall_intro
      variable(S: Set U)
      variable(S': Set U)
      assume(h₁: S =ₛₑₜ S')
