...
axiom is_singleton_def: ∀ (S: (Set U).Particular), is_singleton S ↔ ∃!₍U₎ (x: U.Particular), x ∈ₛₑₜ S
...
def singleton_predicate: CongruentUnaryPredicate (Set U) :=
  ...
  let cong: ∀ (S₁: (Set U).Particular), ∀ (S₂: (Set U).Particular), S₁ =ₛₑₜ S₂ → (pred S₁ ↔ pred S₂) := by forall_intro
    variable(A: (Set U).Particular)
    variable(B: (Set U).Particular)

    -- From set extensionality, A =ₛₑₜ B gives us membership equivalence
    have h₁: ∀ (S₂: (Set U).Particular), A =ₛₑₜ S₂ ↔ (∀ (x: U.Particular), x ∈ₛₑₜ A ↔ x ∈ₛₑₜ S₂) := by forall_elim set_extensionality, A
    ...
    -- Backward: pred B → pred A (symmetric)
