...
-- # Congruence for the inclusion predicate.
--
-- Not auto-derivable because varying either Set requires unfolding `Sets.eq_def`
-- — a Sets-specific fact that the generic auto-cong machinery does not (and
-- should not) know. We prove combined cong manually here; the fibers
-- `supersets_of` and `subsets_of` derive their unary cong for free via the
-- framework's `fiber_first_preserves_binary_congruence` /
-- `fiber_second_preserves_binary_congruence`.
-- Proof by Claude Opus 4.7 (claude-opus-4-7), 2026-05-31
theorem inclusion_cong:
    ∀ (S: Set U), ∀ (S': Set U), ∀ (T: Set U), ∀ (T': Set U),
      S =ₛₑₜ S' → T =ₛₑₜ T' →
        ((∀ (x: U.Particular), S.pred x → T.pred x) ↔ (∀ (x: U.Particular), S'.pred x → T'.pred x)) := by forall_intro
  variable(S: Set U)
  variable(S': Set U)
  variable(T: Set U)
  variable(T': Set U)
...
binary_predicate inclusion : (S : (𝐒𝐞𝐭 U).Particular, S' : (𝐒𝐞𝐭 U).Particular ↦ ∀ (x: U.Particular), S.pred x → S'.pred x) with inclusion_cong
