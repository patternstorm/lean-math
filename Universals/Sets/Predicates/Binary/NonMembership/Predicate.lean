...
-- # Congruence for the non-membership predicate.
--
-- Derived from `mem_cong` by
-- contrapositive: if `S₁.pred x₁ ↔ S₂.pred x₂` then
-- `¬S₁.pred x₁ ↔ ¬S₂.pred x₂`.
-- Proof by Claude Opus 4.7 (claude-opus-4-7), 2026-05-31
theorem not_mem_cong: ∀ (x₁: U.Particular), ∀ (x₂: U.Particular), ∀ (S₁: Set U), ∀ (S₂: Set U),
