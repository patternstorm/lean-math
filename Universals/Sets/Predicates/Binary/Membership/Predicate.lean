...
--
-- The congruence proof is inlined via the `with cong : ... := ...` form. It's
-- not auto-derivable because varying the `S` argument requires unfolding
-- `Sets.eq_def` — a Sets-specific fact that the generic auto-cong machinery
-- does not (and should not) know. `not_mem` reuses this via contrapositive
-- (see `NonMembership/Predicate.lean`).
