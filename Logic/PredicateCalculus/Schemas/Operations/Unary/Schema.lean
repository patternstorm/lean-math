...
-- a congruent binary predicate G(x, y) satisfying: ∀ x y, op(x) =₍U₂₎ y ↔ G(x, y).
--
-- The structure bundles the graph (.ext), the operation function symbol (.op), and the
-- defining axiom (.def). Congruence is derived as a theorem (UnaryOperation.cong) from
-- graph congruence + the defining axiom — never assumed independently.
--
-- ## How to declare a unary operation
--
-- Given a congruent binary predicate G : CongruentBinaryPredicate U₁ U₂, declare:
--
--     axiom my_op_sym : U₁.Particular → U₂.Particular
--     axiom my_op_def : ∀ x y, (my_op_sym x =₍U₂₎ y) ↔ (G.pred x).pred y
--     noncomputable def my_op : U₁ ⟴ U₂ :=
--       { ext := G, op := my_op_sym, def := my_op_def }
--
-- After declaration, the following are available:
--   my_op x        — apply the operation (via CoeFun)
--   my_op.ext      — the graph predicate (reduces to G)
...
structure UnaryOperation(U₁ U₂: Universal): Type where
  ext: CongruentBinaryPredicate U₁ U₂
  op: U₁.Particular → U₂.Particular
  «def»: ∀ (x: U₁.Particular), ∀ (y: U₂.Particular), (op x =₍U₂₎ y) ↔ (ext.pred x).pred y
...
--
-- Given a congruent graph predicate and the defining axiom, congruence follows:
-- 1. By reflexivity + def forward: ext(x₁, op x₁)
-- 2. By graph congruence in 1st arg: ext(x₂, op x₁)
...
theorem UnaryOperation.cong (op: U₁ ⟴ U₂):
  ...
  have h₂: ∀ (y: U₂.Particular), (op x₁ =₍U₂₎ y) ↔ (op.ext.pred x₁).pred y := by forall_elim op.def, x₁
  have h₃: (op x₁ =₍U₂₎ op x₁) ↔ (op.ext.pred x₁).pred (op x₁) := by forall_elim h₂, (op x₁)
  have h₄: op x₁ =₍U₂₎ op x₁ := by forall_elim U₂.eq.refl, (op x₁)
  have h₅: (op.ext.pred x₁).pred (op x₁) := PC₀.deductive_eq_l2r h₃ h₄

  -- Graph congruence in 1st arg: ext(x₁, op x₁) → ext(x₂, op x₁)
  have h₆: ∀ (y: U₁.Particular), ∀ (z: U₂.Particular), x₁ =₍U₁₎ y → ((op.ext.pred x₁).pred z ↔ (op.ext.pred y).pred z) := by forall_elim op.ext.cong, x₁
  have h₇: ∀ (z: U₂.Particular), x₁ =₍U₁₎ x₂ → ((op.ext.pred x₁).pred z ↔ (op.ext.pred x₂).pred z) := by forall_elim h₆, x₂
  have h₈: x₁ =₍U₁₎ x₂ → ((op.ext.pred x₁).pred (op x₁) ↔ (op.ext.pred x₂).pred (op x₁)) := by forall_elim h₇, (op x₁)
  have h₉: (op.ext.pred x₁).pred (op x₁) ↔ (op.ext.pred x₂).pred (op x₁) := by modus_ponens h₈, h₁
  have h₁₀: (op.ext.pred x₂).pred (op x₁) := PC₀.deductive_eq_l2r h₉ h₅

  -- ext(x₂, op x₁) → op x₂ =₍U₂₎ op x₁ via def backward
  have h₁₁: ∀ (y: U₂.Particular), (op x₂ =₍U₂₎ y) ↔ (op.ext.pred x₂).pred y := by forall_elim op.def, x₂
  have h₁₂: (op x₂ =₍U₂₎ op x₁) ↔ (op.ext.pred x₂).pred (op x₁) := by forall_elim h₁₁, (op x₁)
