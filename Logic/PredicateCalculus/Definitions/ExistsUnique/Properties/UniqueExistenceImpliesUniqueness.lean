...
theorem unique_existence_implies_uniqueness {U: Universal} {P: U.Particular → Prop} (h₁: ∃!₍U₎ (x: U.Particular), P x):
  ∀ (x: U.Particular), ∀ (y: U.Particular), P x ∧ P y → x =₍U₎ y := by
    ...
    have h₆: ∀ (x: U.Particular), ∀ (y: U.Particular), P x ∧ P y → x =₍U₎ y := by forall_intro
