...
example {U: Universal} (A: SingletonSet U): Set U := ↑A

-- (2) ↑A where target is `(𝐒𝐞𝐭 ?).Particular` (metavariable) — fails:
example {U: Universal} (A: SingletonSet U): Prop := is_singleton ↑A
...
example {U: Universal} (A: SingletonSet U):
    is_singleton (U:=U) A ↔ ∃!₍U₎ (x: U.Particular), x ∈ₛₑₜ A := by forall_elim is_singleton.def, ↑A
